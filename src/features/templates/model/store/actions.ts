...
import { v4 } from 'uuid';
import { historyStore, nodeManagerStore, nodeSelectionStore, windowStore } from '@/features/board';
import {
	deleteStoredNodes,
	loadStoredNodes,
...
const onChangeTemplate = atom(null, (_, set, templateId: string) => {
	...
	set(windowStore.resetWindowPosition);
	set(nodeSelectionStore.clearSelectedNodes);
	set(historyStore.historyAtom, nodes ? [nodes] : [[]]);
	set(historyStore.historyIndexAtom, 0);
	set(nodeManagerStore.nodesAtom, nodes ?? []);
